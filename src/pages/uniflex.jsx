...
const UnifexChart = () => {
  ...
  const [quantities, setQuantities] = useState([250, 500, 1000, 2000, 3000, 5000, 10000]);
  ...
  const calculateRow = (x, y, index) => {
    ...
    const Q4 = (gsm / 1000 / 1550) * m48 + ((gsm / 1000 / 1550) * m48 * 3) / 100;
    const R4 = Q4 * x * y;
    const AA4 = (R4 + AH4) * P4;
    const S4 = R4 + AA4 + AH4;
    ...
    return {
      ...
      T4, U4, S4, Q4, R4, P4, AA4, AB4, AC4, AD4, AE4, AF4, AG4, AH4, AI4, H4, I4, J4
    };
  };

  const rows = sizeInches.map((size, index) => calculateRow(size.x, size.y, index));
    ...
    <div className="p-4">
      ...
      <div className="text-white mb-10">
        {[110, 100, 90, 80].map((gsmValue) => (
          <button
            key={gsmValue}
            className={`ml-5 p-4 border rounded ${gsm === gsmValue ? 'bg-blue-700' : 'bg-blue-500'}`}
      ...
      <table className="w-[300%] border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-100">
            ...
            {quantities.map(q => (
              <th key={q} className="border border-gray-300 p-2">{q}</th>
            ))}
            ...
            {quantities.map(q => (
              <th key={`process-${q}`} className="border border-gray-300 p-2">{q}</th>
            ))}
            <th className="border border-gray-300 p-2">Pro</th>
            <th className="border border-gray-300 p-2">Process</th>
            <th className="border border-gray-300 p-2 " colSpan={2}>Over Heads</th>
            <th className="border border-gray-300 p-2">Ink</th>
            <th className="border border-gray-300 p-2">Sttich</th>
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={index}>
              <td className="border border-gray-300 p-2">{`${row.sizeFoot.x.toFixed(2)} x ${row.sizeFoot.y.toFixed(2)}`}</td>
              ...
              {row.values.map((value, i) => (
                <td key={i} className="border border-gray-300 p-2">{i === 1 ? row.I4.toFixed(2) : i === 2 ? row.J4.toFixed(2) : value.toFixed(2)}</td>
              ))}
              <td className="p-4"></td>

              <td className="border border-gray-300 p-2">{row.pPercent.toFixed(2)}%</td>
              <td className="border border-gray-300 p-2">{row.Q4.toFixed(3)}</td>
              <td className="border border-gray-300 p-2">{row.R4.toFixed(2)}</td>

              <td className="border border-gray-300 p-2">{row.totalCost.toFixed(2)}</td>
              {row.processValues.map((value, i) => (
                <td key={i} className="border border-gray-300 p-2">{i === 0 ? row.T4.toFixed(2) : i === 1 ? row.U4.toFixed(2) : (value - 6000).toFixed(2)}</td>
              ))}
              <td className="border border-gray-300 p-2">{row.AA4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AB4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AC4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AD4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AE4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AF4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AG4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AH4.toFixed(2)}</td>
              <td className="border border-gray-300 p-2">{row.AI4.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="mt-10">
        ...
        <ul className=''>
          {Object.entries(constants).map(([key, value]) => (
            <li className='pt-2' key={key}>{key.toUpperCase()}: {value}</li>
          ))}
        </ul>
      </div>
    </div>
